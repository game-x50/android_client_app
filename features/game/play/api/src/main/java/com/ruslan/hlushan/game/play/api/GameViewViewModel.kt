...
import com.ruslan.hlushan.game.play.api.calculations.isAllItemsFilled
import com.ruslan.hlushan.game.play.api.calculations.toImmutableNumbersMatrix
import com.ruslan.hlushan.game.play.api.calculations.updateFrom
...
class GameViewViewModel(
    ...
    fun onActionDown(xTouch: Float, yTouch: Float) {
        ...
        ifNotNull(removeNearestNewItemForTouch(xTouch, yTouch)) { area ->
            ...
            movedItemPair?.moved?.leftX = (xTouch - (RectangleArea.MOVED_ITEM_COEFFICIENT_PADDING_X * area.size))
            movedItemPair?.moved?.topY = (yTouch - (RectangleArea.MOVED_ITEM_COEFFICIENT_PADDING_Y * area.size))
        }
    ...
    fun onActionMove(xTouch: Float, yTouch: Float) {
        ifNotNull(movedItemPair?.moved) { movedArea ->
            movedArea.leftX = xTouch - (RectangleArea.MOVED_ITEM_COEFFICIENT_PADDING_X * movedArea.size)
            movedArea.topY = yTouch - (RectangleArea.MOVED_ITEM_COEFFICIENT_PADDING_Y * movedArea.size)
        }
